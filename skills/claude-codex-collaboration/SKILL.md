...
- **The `codex:codex-rescue` wrapper does NOT forward `--model` / `--effort`.** Putting routing flags in the prompt text gets them stripped but not converted into companion arguments — the job silently runs on the `~/.codex/config.toml` defaults (observed: `job.request.model: null`). When a specific model matters, call the companion directly: `codex-companion.mjs task "<prompt>" --background --model <m> --effort <e>`, then **verify it took effect** with `status <job-id> --json` → check `job.request.model` / `job.request.effort`. Never assume a flag landed without reading it back.
- **The wrapper agent can die before submitting the job** (e.g. it hits the host Claude session limit mid-forward). Symptom: the wrapper reports failure or success-with-no-job, and `status --json` shows `running: []` with no new job id. That means nothing was submitted — don't wait; submit directly via the companion and poll yourself (a background loop over `status <job-id> --json` until `completed`/`failed`, then `result <job-id>`).
