...
> A review heuristic from the field: in one session, 3 of 5 spec findings attacked the gap between **intent and acceptance gate** — "the gate passes while the intent fails" (a keyword grep that only covered one language; a keyword check that didn't bind the content to the field that actually renders). When writing gates, bind required content to its **location and shape**, not to keyword presence, and treat gate-shaped findings as substantive rather than nitpicks.
...
- **The wrapper regularly hands back only a job id — that is a normal outcome, not a failure.** The wrapper reports a "120s foreground window", though the observed boundary is not strict wall-time (some 4-minute runs still returned their result inline). When the window does trip, the job moves to background and the wrapper returns "started as `<job-id>`" — sometimes phrased as if it will report back later. **It won't**: the wrapper exits, and its completion notification carries only the job id, never the result. Whenever a job id comes back instead of a result, retain it and poll `status <job-id> --json` until `completed`/`failed`/`cancelled`, then `result <job-id>`. In one field session 3 of 5 wrapper calls behaved this way — treat the inline result as a convenience you cannot rely on.
